          ...
          <div className="grid gap-6 md:grid-cols-2">
            {questions.map((question) => (
              <Card key={question.id} className="cursor-pointer hover:shadow-lg transition-shadow">
                <CardHeader>
                  <CardTitle className="text-xl font-semibold mb-2 truncate">{question.title}</CardTitle>
                  <CardDescription>
                    <span
                      className={`inline-block px-2 py-1 text-xs font-semibold rounded-full ${getDifficultyColor(question.difficulty)} shadow-[0_0_10px_-1px] animate-pulse`}
                    >
                      {question.difficulty}
                    </span>
                  </CardDescription>
                ...
                <CardContent>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{question.description}</p>
                  <Button onClick={() => handleQuestionSelect(question.id)}>Start Challenge</Button>
                </CardContent>
              </Card>
            ))}
          </div>
